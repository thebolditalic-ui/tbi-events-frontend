// /api/track-event-view.js — Vercel serverless function
// Records a detail-page view of an individual event.
// Public; no auth required. Called by the events page when the SPA
// renders a detail view (loadDetailPage) AND on direct loads of /<slug>/.
//
// Body: { event_slug, event_url, session_id, page_url, referrer }
//
// Client dedupes per (event_slug + session_id) in sessionStorage, so the
// same reader bouncing back and forth between the list and a detail
// counts at most once for that detail. Bots and prefetches are dropped
// server-side.
...
module.exports = async function handler(req, res) {
  ...
  if (!eventSlug || !sessionId) {
    return res.status(400).json({ error: 'Missing event_slug or session_id' });
  }
  ...
  try {
    var resp = await fetch(SUPABASE_URL + '/rest/v1/event_views', {
      method: 'POST',
      headers: {
